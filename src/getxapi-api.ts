...
export interface GetxapiSearchOptions {
    apiKey?: string;
    baseUrl?: string;
    fetchImpl?: FetchLike;
    timeoutMs?: number;
...
export function buildGetxapiSearchUrl(
    query: string,
    count: number,
...
export async function searchTweetsWithGetxapi(
    query: string,
    count: number,
    options: GetxapiSearchOptions = {}
): Promise<{ tweets: Tweet[], users: TwitterUser[] }> {
    const apiKey = options.apiKey?.trim();
    if (!apiKey) {
        throw new Error('GETXAPI_API_KEY or GETXAPI_KEY is required when SEARCH_BACKEND=getxapi');
    }
    ...
    try {
        response = await (options.fetchImpl ?? fetch)(
            buildGetxapiSearchUrl(query, count, options.baseUrl),
            {
                headers: {
    ...
    if (!response.ok) {
        ...
        throw new Error(`GetXAPI search failed with HTTP ${response.status}${details}`);
    }
